...
public class iZooto: NSObject {
    ...
    private static var keySettingDetails = [String: Any]()
    ...
    @objc public static func initialisation(
        izooto_id: String, application: UIApplication, iZootoInitSettings: [String: Any]
    ) {
        let bundleName =
        ...
        if izooto_id == nil || izooto_id == "" {
            Utils.handleOnceException(
                bundleName: bundleName, exceptionName: "iZooto app id is not found\(izooto_id)",
                className: "iZooto", methodName: "initialisation", rid: nil, cid: nil, userInfo: nil
            )
            return
        }
        ...
        NetworkManager.shared.sendRequest(request) { result in
            ...
            case .success(let data):
                ...
                do {
                    ...
                    if let groupName = Utils.getGroupName(bundleName: bundleName) {
                        AppStorage.shared.configureAppGroup(groupName)
                        BadgeManager.shared.handleBadgeStatus(
                            responseData.isBadge, bundleName: bundleName)
                    }
                    DispatchQueue.main.async {
                        SettingsManager.shared.handleKeySettingDetails(
                            bundleName: bundleName, keySettingDetails: iZootoInitSettings,
        ...
        if let userPropertiesData = AppStorage.shared.getAnyValue(
            forKey: AppConstant.iZ_USERPROPERTIES_KEY) as? [String: Any]
        {
            UserPropertyManager.shared.addUserProperties(data: userPropertiesData)
        }
        if let eventData = AppStorage.shared.getAnyValue(forKey: AppConstant.KEY_EVENT)
            as? [String: Any],
    ...
    @objc public static func setBadgeCount(badgeNumber: NSInteger) {
        ...
        let bundleName =
            Bundle.main.object(forInfoDictionaryKey: AppConstant.BUNDLE_IDENTIFIER) as? String ?? ""
        AppStorage.shared.configureAppGroup(Utils.getGroupName(bundleName: bundleName) ?? "")

        // If badgeNumber is 1, store the value and a flag into app group shared UserDefaults
        if badgeNumber == 1 {
            if let sharedUserDefaults = UserDefaults(
                suiteName: Utils.getGroupName(bundleName: bundleName))
            {
    ...
    @objc public static func getNotificationFeed(
        isPagination: Bool, completion: @escaping (String?, Error?) -> Void
    ) {
        ...
        if let userID = AppStorage.shared.getString(forKey: AppConstant.REGISTERED_ID),
            let token = AppStorage.shared.getString(forKey: AppConstant.IZ_DEVICE_TOKEN)
        {
            ...
            RestAPI.fetchDataFromAPI(isPagination: isPagination, iZPID: userID) {
                (jsonString, error) in
    ...
    @available(iOS 11.0, *)
    @objc public static func didReceiveNotificationExtensionRequest(
        bundleName: String, soundName: String, isBadge: Bool, request: UNNotificationRequest,
        ...
        if let jsonDictionary = userInfo as? [String: Any] {
            if let aps = jsonDictionary[AppConstant.iZ_NOTIFCATION_KEY_NAME] as? NSDictionary {
                    ...
                    suiteName: Utils.getGroupName(bundleName: bundleName))
                {
                    ...
                    if appId.isEmpty {
                        let errorMessage =
                            "Bundle name mismatch: Please ensure the bundle name in the NotificationService class matches the main app's bundle identifier. A mismatch can affect push notifications, badge count, delivery and impressions."
                        debugPrint(errorMessage)
                        Utils.handleOnceException(
                            bundleName: bundleName,
                            exceptionName: "\(errorMessage) , your bundle name is :\(bundleName)",
                            className: "iZooto", methodName: "didReceiveNotification", rid: nil,
                            cid: nil, userInfo: userInfo)
                    }
                }
                if aps.value(forKey: AppConstant.iZ_ANKEY) != nil {
                    if let userInfoData = userInfo as? [String: Any] {
                        MediationManager.shared.payLoadDataChange(
                            payload: userInfoData, bundleName: bundleName, isBadge: isBadge,
                            isEnabled: isEnabled, soundName: soundName, userInfo: userInfo,
                            bestAttemptContent: bestAttemptContent, contentHandler: contentHandler)
                    }
                } else {
                    //to get all aps data & pass it to commonfu function
                    if let apsDictionary = userInfo[AppConstant.iZ_NOTIFCATION_KEY_NAME]
                        as? NSDictionary
                    {
                        if let notificationData = Payload(dictionary: apsDictionary) {
                            guard notificationData.rid != nil && notificationData.created_on != nil
                            else {
                                debugPrint(AppConstant.IZ_TAG, AppConstant.iZ_KEY_OTHER_PAYLOD)
                                Utils.handleOnceException(
                                    bundleName: bundleName,
                                    exceptionName: "\(AppConstant.iZ_KEY_OTHER_PAYLOD) \(userInfo)",
                                    className: "iZooto", methodName: "didReceive",
                                    rid: notificationData.rid, cid: notificationData.id,
                                    userInfo: userInfo)
                                return
                            ...
                            setupBadgeSoundAndHandleImpression(
                                bundleName: bundleName, isBadge: isBadge,
                                bestAttemptContent: bestAttemptContent,
                                notificationData: notificationData, userInfo: userInfo,
                                isEnabled: isEnabled, soundName: soundName)

                            //Relevance Score
                            self.setRelevanceScore(
                                notificationData: notificationData,
                                bestAttemptContent: bestAttemptContent)
                            if notificationData.fetchurl != nil && notificationData.fetchurl != "" {

                                FetcherAds.shared.handleFetcherNotification(
                                    notificationData: notificationData, bundleName: bundleName,
                                    userInfo: userInfo, bestAttemptContent: bestAttemptContent,
                                    contentHandler: contentHandler)
                            } else {
                                if notificationData != nil {
                                    let firstIndex = notificationData.rid?.prefix(1).first
                                    if firstIndex != "6" && firstIndex != "7" {
                                        notificationReceivedDelegate?.onNotificationReceived(
                                            payload: notificationData)
                                    }

                                    if notificationData.category != ""
                                        && notificationData.category != nil
                                    {
                                        //to store categories
                                        NotificationCategoryManager.shared.storeCategories(
                                            notificationData: notificationData, category: "")
                                        if notificationData.act1name != ""
                                    ...
                                    DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                                        autoreleasepool {
                                            guard (notificationData.alert?.attachment_url) != nil
                                            else { return }
                                            guard
                                                let attachment =
                                                    UNNotificationAttachment.saveImageToDisk(
                                                        bundleName: bundleName,
                                                        cid: notificationData.id,
                                                        rid: notificationData.rid,
                                                        imgUrl: notificationData.alert?
                                                            .attachment_url ?? "",
                                                        userInfo: userInfo, options: nil)
                                            else {
                                                if isEnabled == true {
    ...
    // To handle badgeCount, Sound and call impression
    @objc static func setupBadgeSoundAndHandleImpression(
        bundleName: String, isBadge: Bool, bestAttemptContent: UNMutableNotificationContent,
        notificationData: Payload, userInfo: [AnyHashable: Any]?, isEnabled: Bool, soundName: String
    ) {
        // custom notification sound
        if soundName != "" {
            bestAttemptContent.sound = UNNotificationSound(
                named: UNNotificationSoundName(string: soundName) as String)
        } else {
        ...
        if bundleName != "" {
            ...
            if let userDefaults = UserDefaults(suiteName: groupName) {
                    ...
                    if isFunction {
                        if let sharedUserDefaults = UserDefaults(suiteName: groupName) {
                            ...
                            if badgeCount == 1 {
                                ...
                                bestAttemptContent.badge =
                                    (max(badgeNumber, badgeCount + 1)) as NSNumber
                ...
                if number != nil {
                    handleImpresseionCfgValue(
                        cfgNumber: number ?? 0, notificationData: notificationData,
                        bundleName: bundleName, isSilentPush: false, userInfo: userInfo)
                }
                userDefaults.synchronize()
            } else {
                if isEnabled == true {
                ...
                Utils.handleOnceException(
                    bundleName: bundleName, exceptionName: AppConstant.iZ_APP_GROUP_ERROR_,
                    className: "iZooto", methodName: "setupBadgeSoundAndHandleImpression", rid: rid,
                    cid: cid, userInfo: userInfo)
            }
        }
    }

    //To set relevance score in above iOS 15
    @objc static func setRelevanceScore(
        notificationData: Payload, bestAttemptContent: UNMutableNotificationContent
    ) {
    ...
    // for json aaray
    @objc private static func getParseArrayValue(jsonData: [[String: Any]], sourceString: String)
        -> String
    {
        if sourceString.contains("~") {
            ...
            let value = firstPart.replacingOccurrences(of: "[", with: "").replacingOccurrences(
                of: "]", with: "")
            ...
            case .denied:
                DispatchQueue.main.async {
                    let alert = UIAlertController(
                        title:
                            "Please enable notifications for \(Bundle.main.object(forInfoDictionaryKey: "CFBundleName") ?? "APP Name")",
                        message:
                            "To receive these updates,you must first allow to receive \(Bundle.main.object(forInfoDictionaryKey: "CFBundleName") ?? "APP Name") notification from settings",
                        preferredStyle: UIAlertController.Style.alert)
                    alert.addAction(
                        UIAlertAction(
                            title: " Not Now", style: UIAlertAction.Style.default, handler: nil))
                    alert.addAction(
                        UIAlertAction(
                            title: "Take me there", style: .default,
    ...
    @objc private static func getParseValue(jsonData: [String: Any], sourceString: String) -> String
    {
        ...
        for (index, key) in keys.enumerated() {
            if let indexKey = Int(key), let array = currentData as? [Any],
                array.indices.contains(indexKey)
            {
    ...
    @objc public static func handleForeGroundNotification(
        notification: UNNotification, displayNotification: String,
        ...
        guard let jsonDict = userInfo as? [String: Any],
            let aps = jsonDict[AppConstant.iZ_NOTIFCATION_KEY_NAME] as? NSDictionary
        else {
    ...
    @objc public static func notificationHandler(response: UNNotificationResponse) {
        ...
        if let finalBids = aps[AppConstant.IZ_FETCH_AD_DETAILS] as? [String: Any] {
            NotificationHandlerHelper.shared.handleMediation(
                notificationData: notificationData, userInfo: userInfo, response: response,
                finalBids: finalBids, bundleName: bundleName)
        } else {
            NotificationHandlerHelper.shared.handleStandard(
                notificationData: notificationData, userInfo: userInfo, response: response,
                bundleName: bundleName)
        }
    }

    private static func handleImpresseionCfgValue(
        cfgNumber: Int, notificationData: Payload, bundleName: String, isSilentPush: Bool,
        ...
        if firstDigit == 1 {
            RestAPI.callImpression(
                notificationData: notificationData, pid: pid, token: token, bundleName: bundleName,
                isSilentPush: isSilentPush, userInfo: userInfo)
        }
        ...
        if thirdDigit == 1 {  // Weekly lastView
            if lastDay == nil || lastWeekDay == nil
                || (formattedDate != lastDay && Date().dayOfWeek() == lastWeekDay)
            {
                sharedUserDefault?.set(formattedDate, forKey: AppConstant.IZ_LAST_VIEW_WEEKLY)
                sharedUserDefault?.set(
                    Date().dayOfWeek(), forKey: AppConstant.IZ_LAST_VIEW_WEEKDAYS)
                RestAPI.lastImpression(
                    notificationData: notificationData, pid: pid, token: token, url: url,
                    bundleName: bundleName, userInfo: userInfo)
            }
    ...
    @objc static func clickTrack(
        bundleName: String, notificationData: Payload, actionType: String,
        userInfo: [AnyHashable: Any]?
    ) {
        ...
        guard notificationData.cfg != nil else {
            Utils.handleOnceException(
                bundleName: bundleName, exceptionName: "Both cfg values are nil.",
                className: "iZooto", methodName: "ClickTrack", rid: notificationData.rid,
                cid: notificationData.id, userInfo: userInfo)
            return
        }
        if notificationData.cfg != nil {
            ...
            if secondDigit == 1 {
                RestAPI.clickTrack(
                    bundleName: bundleName, notificationData: notificationData, type: actionType,
                    pid: pid, token: token, userInfo: userInfo ?? nil)
            }
            let formattedDate = Date().getFormattedDate()
            ...
            if thirdDigit == 1 {  //handle weekly lastClick.
                if lastDay == nil || lastWeekday == nil
                    || (lastDay != formattedDate && lastWeekday == Date().dayOfWeek())
                {
                    sharedUserDefault?.set(formattedDate, forKey: AppConstant.IZ_LAST_CLICK_WEEKLY)
                    sharedUserDefault?.set(
                        Date().dayOfWeek(), forKey: AppConstant.IZ_LAST_CLICK_WEEKDAYS)
                    RestAPI.lastClick(
                        bundleName: bundleName, notificationData: notificationData, pid: pid,
                        token: token, url: url, userInfo: userInfo)
                }
                ...
                if tenthDigit == 1 {
                    if formattedDate != lastClickInDay {  // Daily lastClick
                        ...
                        RestAPI.lastClick(
                            bundleName: bundleName, notificationData: notificationData, pid: pid,
                            token: token, url: url, userInfo: userInfo ?? nil)
                    }
                    ...
                    {
                        sharedUserDefault?.set(
                            formattedDate, forKey: AppConstant.IZ_LAST_CLICK_WEEKLY)
                        sharedUserDefault?.set(
                            Date().dayOfWeek(), forKey: AppConstant.IZ_LAST_CLICK_WEEKDAYS)
                        RestAPI.lastClick(
                            bundleName: bundleName, notificationData: notificationData, pid: pid,
            ...
            Utils.handleOnceException(
                bundleName: bundleName, exceptionName: "No CFG Key defined \(userInfo)",
                className: "iZooto", methodName: "ClickTrack", rid: notificationData.rid,
                cid: notificationData.id, userInfo: userInfo)
        }
    }
    ...
    @objc static func onHandleInAPP(
        response: UNNotificationResponse, actionType: String, launchURL: String
    ) {
    ...
    // handle the borwser
    @objc static func onHandleLandingURL(
        response: UNNotificationResponse, actionType: String, launchURL: String
    ) {
        let userInfo = response.notification.request.content.userInfo
        if let apsDictionary = userInfo[AppConstant.iZ_NOTIFCATION_KEY_NAME] as? NSDictionary {
            ...
            if let inAppValue = notifcationData?.inApp, inAppValue.contains("0"),
                !inAppValue.isEmpty
            {
    ...
    @objc static func handleClicks(response: UNNotificationResponse, actionType: String) {
        let userInfo = response.notification.request.content.userInfo
        if let notificationDictionary = userInfo[AppConstant.iZ_NOTIFCATION_KEY_NAME]
            as? NSDictionary
        {
    ...
    @objc public static func addEvent(eventName: String, data: [String: Any]) {
        guard !eventName.isEmpty else { return }
        let returnData = Utils.dataValidate(data: data)
        if let theJSONData = try? JSONSerialization.data(
            withJSONObject: returnData, options: .fragmentsAllowed),
            let validateData = String(data: theJSONData, encoding: .utf8)
        {
            let bundleName =
                Bundle.main.object(forInfoDictionaryKey: AppConstant.BUNDLE_IDENTIFIER) as? String
                ?? ""
            if let token = AppStorage.shared.getString(forKey: AppConstant.IZ_DEVICE_TOKEN),
                !token.isEmpty,
                let pid = AppStorage.shared.getString(forKey: AppConstant.REGISTERED_ID)
            {
                RestAPI.callEvents(
                    bundleName: bundleName, eventName: Utils.eventValidate(eventName: eventName),
                    data: validateData as NSString, pid: pid, token: token)
            } else {
    ...
    @objc public static func promptForPushNotifications() {
        if #available(iOS 10.0, *) {
            UNUserNotificationCenter.current().delegate =
                appDelegate as? UNUserNotificationCenterDelegate
        }
        if #available(iOS 10.0, *) {
            ...
            apsDictionary["content-available"] as? Int == 1
        {
            if let notificationData = Payload(dictionary: apsDictionary) {
                ...
                if let intNum = number {
                    handleImpresseionCfgValue(
                        cfgNumber: intNum, notificationData: notificationData,
                        bundleName: Bundle.main.bundleIdentifier ?? "", isSilentPush: true,
                        userInfo: userInfo)
                } else {
                    Utils.handleOnceException(
                        bundleName: bundleName, exceptionName: "CFG value :\(cfgValue) .",
                        className: "iZooto", methodName: "handleSilentPushNotification",
                        rid: notificationData.rid, cid: notificationData.id, userInfo: userInfo)
                }
            }
        } else {
            Utils.handleOnceException(
                bundleName: bundleName,
                exceptionName: "Failed to convert payload in NSDistionary DATA: \(userInfo)",
                className: "iZooto", methodName: "handleSilentPushNotification", rid: nil, cid: nil,
                userInfo: userInfo)
        }
    }
...
extension UNNotificationAttachment {
    ...
    static func saveImageToDisk(
        bundleName: String, cid: String?, rid: String?, imgUrl: String,
        userInfo: [AnyHashable: Any]?, options: [NSObject: AnyObject]?
        ...
        do {
            ...
            let attachment = try UNNotificationAttachment(
                identifier: uniqueIdentifier, url: fileURL, options: options)
            ...
            Utils.handleOnceException(
                bundleName: bundleName,
                exceptionName: "mediaUrl : \(imgUrl) ErrorMessage : \(error.localizedDescription)",
                className: AppConstant.IZ_TAG, methodName: "saveImageToDisk", rid: rid, cid: cid,
                userInfo: userInfo)
        }
...
@objc public protocol iZootoNotificationOpenDelegate: NSObjectProtocol {
    func onNotificationOpen(action: [String: Any])
}
